...
var Field = require('./field-facade');
...
class FieldController extends Controller {
    ...
    create(req, res, next) {
        this.model.create(req.body)
            ...
            .then(doc => {
                return Document.find({ category: doc.category })
                    .then(documents => {
    ...
    remove(req, res, next) {
        this.model.remove(req.params.id)
            .then(doc => {
                ...
                Document.find({ category: doc.category })
                    .then(documents => {
                        return async.eachSeries(documents, (document, asyncdone) => {
                            ...
                            Document.update({_id: document._id}, document)
                                .then(result => asyncdone())
                                .catch(err => asyncdone(null, err));
                        }, (err) => {
                            if (err) next(err)
...
module.exports = new FieldController(Field);