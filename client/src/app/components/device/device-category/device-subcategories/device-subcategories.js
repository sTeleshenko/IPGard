(function () {
  ...
  function deviceSubcategories(Documents, toastr, $scope) {
        ...
        .then(function (response) {
          ...
          response.data.forEach(function (document) {
            for(var i = 0; i < document.fields.length; i++){
              if(document.fields[i].field._id === vm.category.subcategoryField && document.fields[i].useAsSubcategory){
    ...
    vm.filterDocuments = function () {
      if(vm.subcategories && vm.subcategories.length && vm.documents) {
